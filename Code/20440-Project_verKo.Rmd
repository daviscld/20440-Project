...
library(edgeR)
mcnts <- round(2^mcounts) # retrieve raw cpm values, rounding to nearest int
my <- DGEList(counts=t(mcnts), group=mID)
...
topN <- 20 #round(topNPerc*ncol(mcnts))
et <- exactTest(my)
degs <- topTags(et, n=topN, sort.by = "logFC", p.value=p_cutoff)
