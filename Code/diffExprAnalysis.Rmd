...
### Human data
hgeneList <- full_results_human$'P.Value'
names(hgeneList) <- full_results_human$ID
...
# Visualize GO term distribution over GO graph; manually saved from Rstudio
hGOgraph <- showSigOfNodes(hGOdata, score(resultKS.elim), firstSigNodes = 5, useInfo = 'all')
...
df_dot <- df_dot[1:20,]
m_dotplt <- dotchart(as.numeric(df_dot$elimKS), labels=df_dot$Term, cex=.7,
                     main="Mouse enrichment for GO terms", xlab="KS elim pval")

### Human dotplot
df_dot <- hallRes[,c("Term","elimKS")]
df_dot <- df_dot[1:20,]
h_dotplt <- dotchart(as.numeric(df_dot$elimKS), labels=df_dot$Term, cex=.7,
